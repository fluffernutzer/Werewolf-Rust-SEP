                    ...
                    match client_message {
                        ...
                        ClientMessage::SeherAction { direction } => {
                            if let Phase::SeherPhase = game.phase {
                                 /*let Some(rolle) = game.seher_schaut(&direction.target) {
                                    game.last_seher_result = Some((direction.target.clone(), rolle));*/
                                    println!("Der Seher hat geschaurt");
                                    game.phase_change(); //Achtung muss geändert werden sobald Seher richtig implementiert 
                                    game.runden +=1;
