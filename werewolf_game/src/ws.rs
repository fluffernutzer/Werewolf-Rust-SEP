...
pub async fn handle_message(
    ...
    match client_message {
        ...
        ClientMessage::EndGame => {
            ...
            if let Some(endgame_tx) = recv_state.endgame_signal.lock().await.take() {
                let _ = endgame_tx.send(());
            }
        }
        ...
        ClientMessage::TagAction { direction } => {
            if let Phase::Tag = game.phase {
                    ...
                    .any(|p| p.name == direction.target.clone().unwrap() && p.lebend)
                {
                    ...
                    log::info!("Ziel bereits tot!")
                };
            } else {
                log::info!("Gerade kann niemand gelynched werden");
            }
        }
        ...
        ClientMessage::SeherAction { actor: _, target } => {
            if let Phase::SeherPhase = game.phase {
                if let Ok(rolle) = game.seher_schaut(&target) {
                    game.last_seher_result = Some((target.clone(), rolle));
                }
            } else {
...
async fn handle_vote(
    ...
    if let Some(_player) = game.players.iter_mut().find(|p| p.name == target) {
        if let Some(_player) = game
            .players
            .iter_mut()
            .find(|p| p.name == actor && p.has_voted)
        {
...
mod tests {
    ...
    async fn add_user_msg() {
        ...
        let state = AppState {
            ...
            tx,
            endgame_signal: Arc::new(Mutex::new(None)),
        };
        ...
        assert!(
            !*state.game_started.lock().await,
            "Spiel sollte noch nicht gestartet sein"
        );
    }
    ...
    async fn not_existing_player_gets_lynched() {
        ...
        let result = handle_vote(
            &mut game,
            "Nutzer".to_string(),
            "NichtNutzer".to_string(),
            ActionKind::DorfLyncht,
        )
        .await;
        ...
        assert_eq!(
            result,
            Err("Spieler nicht gefunden oder nicht lebendig".to_string())
        );
    }
    ...
    impl FromStr for ClientMessage {
        ...
        fn from_str(s: &str) -> Result<Self, Self::Err> {
            ...
            match parts.as_slice() {
                ...
                ["AddUser", username] => Ok(ClientMessage::AddUser {
                    username: username.to_string(),
                }),
                ["IngameBereit", username] => Ok(ClientMessage::IngameBereit {
                    username: username.to_string(),
                    ready: true,
                }),
                ["ReadyStatus", username, _, ready] => {
                    let ready = ready
                        .parse::<bool>()
                        .map_err(|_| "Invalid bool for ready status")?;
                    Ok(ClientMessage::ReadyStatus {
                        username: username.to_string(),
                        ready,
                    })
                }
                ...
                ["Hexe", hexenaktion, actor, extra_target] => {
                    ...
                    Ok(ClientMessage::HexenAction {
                        ...
                        hexen_aktion: hexenaktion,
                        extra_target: extra_target.to_string(),
                    })
                }
    ...
    async fn test_game_one_werwolf_winner() {
        ...
        for clientmessage in spielablauf {
            ...
            let _phase_print = game.phase.clone();
    ...
    async fn test_game_two_wrong_timed_action() {
        ...
        let _remaining: Vec<String> = game
            .players
            .iter()
            .filter(|p| p.lebend)
            .map(|p| p.name.clone())
    ...
    #[tokio::test]
    async fn test_game_reset_game() {
        //Same game as test_game_two_village_winner but last action is reset-call -> winner gets deleted
        let state = AppState {
            tx: broadcast::channel(10).0,
            game: Arc::new(Mutex::new(Game::new())),
            game_started: Arc::new(Mutex::new(false)),
    ...
    #[tokio::test]
    async fn critical_votes_game3() {
        //Testing for handling votes of same amount. e.g 5 for A and 5 for B
        let state = AppState {
            tx: broadcast::channel(10).0,
            game: Arc::new(Mutex::new(Game::new())),
            game_started: Arc::new(Mutex::new(false)),
